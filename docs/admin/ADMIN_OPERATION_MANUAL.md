...
5. [用户管理 (User Management)](#五-用户管理)
6. [工单管理 (Ticket Management)](#六-工单管理)
...
79: 
80: > **注意**: 商品创建完成后，请前往 **SKU 规格与库存** (`products/skus`) 或 **共享规格库** (`products/shared-sku`) 为该商品配置具体的规格和价格。商品本身仅作为展示容器。
...
后台管理员账号管理模块，包含账号 CRUD 和部门权限配置。

### 5.1 用户列表 (`users/accounts`)

展示所有后台管理员账号信息。

*   **功能**:
    *   **新增用户**: 创建新的后台管理员账号（自动同步 Supabase Auth）。
    *   **编辑用户**: 修改用户名称、所属部门。
    *   **状态控制**: 一键 `启用/禁用` 用户。禁用后该账号无法登录后台。
    *   **删除用户**: 删除管理员账号（同时删除关联的 Auth 用户）。
...
2.  填写以下信息:
    *   **用户名称**: 管理员的显示名称。
    *   **绑定邮箱**: 用于登录后台的邮箱地址（唯一）。
    *   **密码**: 初始登录密码（可后续使用验证码免密登录）。
    *   **所属部门**: 选择部门以配置该用户的页面访问权限。
3.  点击 **【确认】** 完成创建。

> **注意**: 创建成功后，系统会自动在 Supabase Auth 中创建对应的认证用户，新用户可立即使用邮箱+密码登录。

### 5.3 部门管理 (`users/departments`)

部门用于分组管理后台用户权限。

*   **创建部门**: 点击 **【新增部门】**，填写部门名称。
*   **权限配置**: 勾选该部门允许访问的后台页面（如 订单管理、商品管理 等）。
*   **删除部门**: 仅允许删除无成员的空部门。

> **特殊部门**: `超级管理员` 部门拥有全部页面权限，不受勾选限制。
...
#### B. 验证码登录
*   输入邮箱 → 点击 **【发送验证码】** → 输入邮箱收到的 6 位验证码 → 登录。
*   无需记忆密码，更安全。

> **安全机制**: 非 `admin_users` 表中的用户即使拥有 Supabase Auth 账号也无法登录后台。
...
*   **关联订单展示**: 列表直接显示用户可见的 `订单号` (order_no) 和商品名称。
    *   点击关联订单号可 **一键复制**。
*   **状态筛选**: 支持按 `处理中` (Processing)、`已解决` (Resolved) 和 `全部` 状态页签切换。
*   **清理图片**: 支持一键清理 7 天前已结单工单的历史图片附件，释放存储空间。

### 6.2 工单处理 (新版弹窗)

点击列表右侧的 **【处理】** 按钮或工单号，打开处理弹窗。采用高效的三栏布局：

#### A. 左侧信息栏
*   **核心状态**: 顶部醒目显示当前工单状态。
*   **用户信息**: 显示提交人的头像、8位 UID 和邮箱。
*   **关联订单快照 (Snapshot)**:
    *   展示下单时的 **商品头图**、**名称** 和 **规格**。
    *   **金额**: 订单实付金额。
    *   **剩余时间**: 自动计算并未过期订单的服务有效期剩余时长（如：`29天 23小时`）。

#### B. 右侧对话流
*   **气泡对话**: 清晰区分 `客` (用户) 和 `服` (管理员) 的消息记录。
*   **图片预览**: 点击图片附件可放大查看。

#### C. 底部操作区
*   **快捷回复**: 输入内容后按 `Cmd + Enter` (Mac) 或 `Ctrl + Enter` (Win) 快速发送。
*   **标记已解决**: 点击绿色按钮 **【标记已解决】**，工单状态将变更为 `resolved`，且用户侧会收到结单通知，无法再继续回复。

> **注意**: 一旦标记为已解决，该工单的对话将锁定。如需重新开启，需用户再次提交新工单。
