...
> **适用路径**: `pages/profile/order/[id].vue` (PC端), `pages/mobile/profile/order/[id].vue` (移动端)
> **更新日期**: 2026-02-04
...
| **取消退款** | `refunding` 状态 且 有待审核申请 | 弹窗确认取消 |
| **申请退款** | `pending_delivery` 或 `active` (非 OneTime) 且 无待审核申请 且 取消次数<3 | 弹窗退款 |
| **退款已达上限** | `pending_delivery` 或 `active` 且 取消次数>=3 | 禁用按钮 |

#### 退款按钮状态机
```
  ┌─────────────────────────────────────────────────────────┐
  │  正常状态 (active/pending_delivery)                     │
  │                                                         │
  │  取消次数 < 3 → 显示 [申请退款]                          │
  │  取消次数 >= 3 → 显示 [退款已达上限] (禁用)              │
  └───────────────────────┬─────────────────────────────────┘
  ...
  │                                                         │
  │  显示 [取消退款]                                        │
  │  内容区域: 显示 MobileRefundingCard 退款处理中卡片       │
  │            隐藏交付内容和使用说明                        │
  └───────────────────────┬─────────────────────────────────┘
                          │ 点击取消退款 (弹窗确认)
                          ▼
...
## 三、交付内容 (Fulfillment) - 按类型和状态区分

### 3.0 退款处理中 (refunding)
**组件**: `MobileRefundingCard.vue`
*   隐藏所有交付内容和使用教程
*   显示退款处理中卡片，包含：申请时间、退款原因、提示文案
...
*   **显示条件**: `cdks.account_data` 中存在图片 URL，且订单状态不为 `refunding`。
*   **交互**: 点击图片可预览大图。
...
| 文件 | 说明 |
|------|------|
| `pages/profile/order/[id].vue` | PC端订单详情主页面 |
| `pages/mobile/profile/order/[id].vue` | 移动端订单详情主页面 |
...
| `components/mobile/order/MobileRefundingCard.vue` | 移动端退款处理中卡片 |
| `components/mobile/order/MobileCancelRefundSheet.vue` | 移动端取消退款弹窗 |
| `components/pc/order/OrderActions.vue` | PC端操作按钮组 |
