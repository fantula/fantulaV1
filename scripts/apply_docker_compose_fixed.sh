...
SERVER_USER="root"
LOCAL_FILE="docker-compose.yml.broken" 
REMOTE_FILE="/opt/supabase/docker/docker-compose.yml"

echo "🔧 Uploading Fixed Docker Compose to $SERVER_IP..."

scp $LOCAL_FILE $SERVER_USER@$SERVER_IP:/tmp/docker-compose.yml.fixed
...
ssh $SERVER_USER@$SERVER_IP "bash -s" << 'EOF'
    ...
    # Backup existing (again, to be safe)
    cp /opt/supabase/docker/docker-compose.yml /opt/supabase/docker/docker-compose.yml.broken.$(date +%s)
    
    # Move new file
    mv /tmp/docker-compose.yml.fixed /opt/supabase/docker/docker-compose.yml
