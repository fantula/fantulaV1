...
BEGIN
    ...
    IF current_config IS NOT NULL THEN
        FOR key, val IN SELECT * FROM jsonb_each(current_config)
        LOOP
            -- If value is boolean, convert to object
            IF jsonb_typeof(current_config -> key) = 'boolean' THEN
                new_config := jsonb_set(
                    new_config, 
                    ARRAY[key], 
                    jsonb_build_object(
                        'enable', val, 
                        'template', COALESCE(default_templates ->> key, '')
                    )
                );
            ELSE
                -- Already object or other, keep as is
                 new_config := jsonb_set(new_config, ARRAY[key], current_config -> key);
            END IF;
        END LOOP;
        -- Update the setting
        UPDATE public.system_settings 
        SET value = new_config::text 
        WHERE key = 'customer_notification_config';
    END IF;
END $$;
