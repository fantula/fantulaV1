<template>
  <PageTipHeader title="激活码订单详情" type="info" />
  <AdminActionCard>
    <template #actions>
...
onMounted(async () => {
  ...
  const res = await adminOrderApi.getCdkeyOrderDetail(id)
  if (res.success) order.value = res.data
})

function goBack() { router.back() }
function openEdit() { router.push(`/manager_portal/orders/cdkey/post?id=${order.value.id}`) }
</script>
