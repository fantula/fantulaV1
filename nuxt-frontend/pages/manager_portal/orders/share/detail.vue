<template>
  <PageTipHeader title="账号合租订单详情" type="info" />
  <AdminActionCard>
    <template #actions>
...
onMounted(async () => {
  ...
  const res = await adminOrderApi.getShareOrderDetail(id)
  if (res.success) order.value = res.data
})

function goBack() { router.back() }
function openEdit() { router.push(`/manager_portal/orders/share/post?id=${order.value.id}`) }
</script>
