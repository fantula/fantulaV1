/**
 * 后台路由配置 - 唯一真理源 (Single Source of Truth)
 * 
 * 所有后台路由前缀和路径都从这里生成。
 * 如需更改后台路径前缀（如 /admin → /manager_portal），只需修改 ADMIN_PREFIX。
 */

export const ADMIN_PREFIX = '/manager_portal'

/**
 * 生成后台路由路径
 * @example adminRoute('cdk/virtual') → '/manager_portal/cdk/virtual'
 */
...
export const adminRoutes = {
    ...
    productSpecs: (id: string) => adminRoute(`products/specs/${id}`),
    ...
    cdkEdit: (id: string) => adminRoute(`cdk/edit/${id}`),
