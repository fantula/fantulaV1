...
import { o as picture_filled_default, an as arrow_up_default, a9 as arrow_down_default, _ as delete_default, ad as picture_default, q as document_default } from './index-DNnPa_Q9.mjs';
import { A as AdminImageSelector } from './AdminImageSelector-BwNJvP03.mjs';
  ...
  const submitProduct = async () => {
    ...
    await formRef.value.validate(async (valid) => {
      ...
      try {
        ...
        let res;
        if (form.id) {
          res = await adminProductApi.updateProduct(form.id, data);
        } else {
          res = await adminProductApi.createProduct(data);
        }
        if (res.success) {
          const createdProduct = res.product;
          if (!form.id && form.copySkus.length > 0 && createdProduct) {
            const skuRes = await adminProductApi.createProductSkus(createdProduct.id, form.copySkus);
...
//# sourceMappingURL=edit-DUbaZB6X.mjs.map
