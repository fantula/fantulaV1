...
const adminCdkApi = {
   ...
   */
  async validateCdkCandidates(candidates, skuIds = []) {
    ...
    if (skuIds.length > 0) {
      const client = getSupabaseClient();
      const { data: mappings } = await client.from("cdk_sku_map").select("cdk:cdks!inner(code, cdk_type)").in("sku_id", skuIds).eq("cdk.cdk_type", "one_time");
      if (mappings) {
        mappings.forEach((m) => {
          var _a;
          if ((_a = m.cdk) == null ? void 0 : _a.code) existingCodes.add(m.cdk.code);
        });
      }
    }
   ...
   */
  async getVirtualBoundSkuIds() {
    const client = getSupabaseClient();
    const { data } = await client.from("cdk_sku_map").select("sku_id, cdk:cdks!inner(cdk_type)").eq("cdk.cdk_type", "virtual");
    if (!data) return [];
...
//# sourceMappingURL=cdk-DYhNEa5O.mjs.map
