...
const adminProductApi = {
  ...
  async getSkusByIds(ids) {
    const client = getSupabaseClient();
    const { data, error } = await client.from("product_sku_map").select("sku:product_skus(*), product:products(product_name)").in("sku_id", ids);
    if (error) return { success: false, error: error.message };
    const skus = data.map((item) => {
      var _a;
      return {
        ...item.sku,
        productName: (_a = item.product) == null ? void 0 : _a.product_name
      };
    }).filter((s) => s.id);
...
//# sourceMappingURL=product-CmEpuoIC.mjs.map
